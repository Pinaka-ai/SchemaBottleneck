...
class MoralityEditMatchMetric(BaseMetric):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.prompt_path = kwargs["prompt_path"]
        self.separator = kwargs["separator"]
        self.openai_api_key = kwargs.get("openai_key", "")
        ...
        self.prompt_folder = kwargs.get("prompt_folder", "")

        # self.aspect_cache = AspectCache(cache_file='cache.jsonl', save_every=5)
        if self.prompt_folder:
            self.prompt_folder = f'{self.prompt_folder}/{self.model_name.replace("/", "_")}'

            with open(f'{self.prompt_folder}/scores.txt', 'r') as f:
                self.scores_prompt = f.read()

            with open(f'{self.prompt_folder}/moral.txt', 'r') as f:
                self.moral_prompt = f.read()
    ...
    def get_diversity_penalty(self, aspects):
        ...
        return (np.maximum(0, similarity_scores - SIMILARITY_THRESHOLD) * 4).sum() / len(aspects)
    ...
    def compute(
                ...
                if feedback_pred:
                    input_wfeed.append(
                        (
                            ...
                            # + feedback_pred
                            # + "\n"
                            self.scores_prompt.replace("{{scenario}}", input_text).replace("{{aspects}}", feedback_pred)
                        )
                    )
                else:
        ...
        for inp, cached_res in zip(input_wfeed, cached_wfeed):
            if not inp and not cached_res:
                ...
                metric_dict = {
                    "custom_metrics/moralityeditmatch_mse": (None, -64.0)
                }
                return metric_dict
        ...
        for obj in gpt_call_1:
            ...
            final_str = "\n".join(arr)
            new_message = self.moral_prompt.replace("{{virtue set}}", final_str)
        ...
        print(f'\n\nThis is the final model\'s output, ', final_preds)
        ...
        if self.save_path != "" and split_name in ["test", "val"]:
            save_path = os.path.join(
                self.save_path, f"{split_name}_moralityeditmatch_{epoch}.json"
            )
        ...
        for k, score in scores.items():
            metric_dict.update({f"custom_metrics/moralityeditmatch_{k}": (None, score)})
        return metric_dict


class MoralityEditMatch(RewardFunction):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__()
        self.tokenizer_config = kwargs["tokenizer"]
        self.tokenizer = build_tokenizer(self.tokenizer_config)
        self.metric = MoralityEditMatchMetric(**kwargs["metric"])
    ...
    def __call__(
        ...
        if done:
            ...
            reward = metric_dict[
                f"custom_metrics/moralityeditmatch_{self.metric.downstream_metric_name}"
            ][-1]
            return reward
...
if __name__ == "__main__":
    ...
    metric = MoralityEditMatch(**kwargs)
    ...
    metric = MoralityEditMatchMetric(**args)
